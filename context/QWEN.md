...
│  └──────────────────┘  └──────────────────┘                          │
│                                                                         │
│  Skills: 26 | Agents: 22 | Commands: 6 | MCP Tools: 18 + Memory      │
└─────────────────────────────────────────────────────────────────────────┘
```
...
4. **Cache**: Store findings to `$SESSION_DIR/docs/`
...
- ALWAYS create a redesign analysis report at `$SESSION_DIR/redesign-analysis.md`
...
Each `/orchestrator` invocation creates a new isolated session with its own directory. Previous sessions are **preserved, never deleted**.
...
- ALL state writes go to `$SESSION_DIR/`
- NEVER modify files from archived sessions
- NEVER delete session directories
- Each session's data is IMMUTABLE after that session ends
...
2. **READ** `$SESSION_DIR/memory.md` for session state
3. **READ** current TodoWrite state for task status
4. **READ** latest checkpoint in `$SESSION_DIR/checkpoints/`
5. **VERIFY** file state matches recorded state
...
## Shared State Directory (Session-Isolated)

Each session has its own isolated state directory under `.qwen-orchestrator/sessions/<session-id>/`.
